// app/page.tsx (ниже героя)
import Link from 'next/link';

function SectionShell({ id, title, children }: { id: string; title: string; children: React.ReactNode }) {
  return (
    <section id={id} className="relative mx-auto max-w-6xl px-4 py-16">
      <h2 className="text-2xl md:text-3xl font-bold mb-4">{title}</h2>
      <div className="text-white/80 mb-6">{children}</div>
      <div className="flex gap-3">
        <a href={`/#${id}`} className="rounded-xl border border-white/15 px-3 py-2 text-sm">Scroll here</a>
        <Link href={`/${id}`} className="rounded-xl bg-white/10 px-3 py-2 text-sm hover:bg-white/15">Open full page →</Link>
      </div>
    </section>
  );
}

// ...в JSX страницы (после Hero/плиток)
